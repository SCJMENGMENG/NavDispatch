...
// 替换订单中间部分为*
const replaceMidStr = (code) =>{
    let reg = /^([\da-zA-Z]{4})([\da-zA-Z]*)([\da-zA-Z]{4})$/;
    code = code.replace(reg,function(all, top, middle, bottom) {
        // console.log('code==',all,top,middle,bottom);
        return top + middle.replace(/([\da-zA-Z])/g, "*") + bottom;
    });
    return code;
}
...
/*
* 正则表达式校验
* 使用：if (!Validator.isNewPassword(this.state.oldPassword)) Toast.show('原密码格式错误，请重新输入')
*/
